...
# Assign config vars
base_url = conf.get("GLOBAL", "base_url")
...
User_Agent = conf.get("GLOBAL", "user_agent")
...
# Query modifiers supported by zKillboard
valid_modifiers = (
    "kills",
    "losses",
    "w-space",
...
# Overriding class to return custom errors
...
class Query(object):
    ...
    # Find modifiers passed into query (api-only, asc, etc)
    def parseQueryArgs(self, queryArgs):  # Would prefer to also do internal validation
        ...
        for item in arg_list:
            ...
            if item.isdigit():
                self.queryElements[previous_item] = item
...
def fetchResult(zkb_url):
    ...
    for tries in range(0, retry_limit):
        ...
        try:
            ...
            conn_sleep_time = int(http_header["X-Bin-Seconds-Between-Request"])
        except KeyError as E:
            snooze_routine += 1
            try:
...
def crash_recovery():
    ...
    zkb_args = query_address.split(base_url)
...
def main():
    newQuery2 = Query("2013-12-20", "corporationID/1894214152/")
    pdb.set_trace()
    # _crash_recovery()
